...
def mostrar_todas_tareas(tareas):
    if tareas:
            ...
            for tarea in tareas:
                ...
                print(Fore.CYAN + f"🆔 {tarea.id} - {tarea.title} - Usuario: {tarea.user}: descripcion: {tarea.description}, categoria: [{cat}], prioridad: {tarea.priority}, fecha de creación [{tarea.createDate}], fecha de edición [{tarea.lastEditDate}], [{estado}].")
    else:
...
def mostrar_tarea(task):
    ...
    print(Fore.CYAN + f"🆔 {task.id} - {task.title} - Usuario: {task.user}: descripcion: {task.description}, categoria: [{cat}], prioridad: {task.priority}, fecha de creación [{task.createDate}], fecha de edición [{task.lastEditDate}], [{estado}].")
